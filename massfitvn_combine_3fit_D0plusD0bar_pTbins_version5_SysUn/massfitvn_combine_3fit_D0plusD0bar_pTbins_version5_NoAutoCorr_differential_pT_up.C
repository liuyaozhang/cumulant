...
void massfitvn_combine_3fit_D0plusD0bar_pTbins_version5_NoAutoCorr_differential_pT_up(Double_t vnYrangeMin = -0.2, Double_t vnYrangeMax = 0.6, Bool_t isForward=false)
{
   ...
   isV24HF=true;
    ...
    TFile* file1 = TFile::Open("/afs/cern.ch/user/z/zhangli/public_work/Scalar-product/batch/TestOutPutDir/data_build17_AverageParticle_ExactCumulant_MVA_selection_MC_new_Centtable_Jun30/v2_vs_pT/systematic_uncertainty/Centable/95percent_data_default_8pTbins_centralitytable_up/PbPbMB.ExactCumulant_total.root");
    ofile.cd();
	...
	TF1* f = new TF1(Form("f_%d",i),"[0]*([5]*([4]*TMath::Gaus(x,[1],[2]*(1.0 +[6]))/(sqrt(2*3.14159)*[2]*(1.0 +[6]))+(1-[4])*TMath::Gaus(x,[1],[3]*(1.0 +[6]))/(sqrt(2*3.14159)*[3]*(1.0 +[6])))+(1-[5])*TMath::Gaus(x,[8],[7]*(1.0 +[6]))/(sqrt(2*3.14159)*[7]*(1.0 +[6]))) + [9] + [10]*x + [11]*x*x + [12]*x*x*x", fit_range_low, fit_range_high);
        ...
        f->SetParameter(11, 10000);
        f->SetParameter(12, 1000);
