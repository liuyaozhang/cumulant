...
void massfitvn_combine_3fit_D0plusD0bar_pTbins_version5_NoAutoCorr_differential_pT_prompt(Double_t vnYrangeMin = -0.2, Double_t vnYrangeMax = 0.6, Bool_t isForward=false)
{
   ...
   isV24HF=true;
	...
	TF1* f = new TF1(Form("f_%d",i),"[0]*([5]*([4]*TMath::Gaus(x,[1],[2]*(1.0 +[6]))/(sqrt(2*3.14159)*[2]*(1.0 +[6]))+(1-[4])*TMath::Gaus(x,[1],[3]*(1.0 +[6]))/(sqrt(2*3.14159)*[3]*(1.0 +[6])))+(1-[5])*TMath::Gaus(x,[8],[7]*(1.0 +[6]))/(sqrt(2*3.14159)*[7]*(1.0 +[6]))) + [9] + [10]*x + [11]*x*x + [12]*x*x*x", fit_range_low, fit_range_high);
        ...
        f->SetParameter(9, 2.91719e+06);
        f->SetParameter(10, -1.21223e+06);
        f->SetParameter(11, -702343);
        f->SetParameter(12, 322376);
