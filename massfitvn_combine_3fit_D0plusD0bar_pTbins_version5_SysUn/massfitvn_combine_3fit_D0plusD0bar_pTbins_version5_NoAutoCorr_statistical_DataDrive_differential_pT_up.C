...
TString folderForPlots = "../f_update_build17_ExactCumulant_"+detector[number_detector]+"_NoAutoCorr_update_Jun30/v2_vs_pT/systematic_uncertainty/Centable/95percent_data_default_8pTbins_centralitytable_up";
//tracker:
//|y|<1
void massfitvn_combine_3fit_D0plusD0bar_pTbins_version5_NoAutoCorr_statistical_DataDrive_differential_pT_up(Double_t vnYrangeMin = -0.2, Double_t vnYrangeMax = 0.6, Bool_t isForward=false)
{
   ...
   for(Int_t icent=0; icent<n_centbin; icent++){//icent    
	...
	TF1* f = new TF1(Form("f_%d_%d_%d", icent, iset, i),"[0]*([5]*([4]*TMath::Gaus(x,[1],[2]*(1.0 +[6]))/(sqrt(2*3.14159)*[2]*(1.0 +[6]))+(1-[4])*TMath::Gaus(x,[1],[3]*(1.0 +[6]))/(sqrt(2*3.14159)*[3]*(1.0 +[6])))+(1-[5])*TMath::Gaus(x,[8],[7]*(1.0 +[6]))/(sqrt(2*3.14159)*[7]*(1.0 +[6]))) + [9] + [10]*x + [11]*x*x + [12]*x*x*x", fit_range_low, fit_range_high);
        ...
        //f->SetParameter(9, 100000);
        //f->SetParameter(10, -100000);
        //f->SetParameter(11, 50000);
        //f->SetParameter(12, 20000);
    ...
    ofout << "icent=" << "," << icent << endl; 
     ...
     TFile* f_sumw = new TFile("/afs/cern.ch/user/z/zhangli/public_work/Scalar-product/f_update_build17_ExactCumulant_"+detector[number_detector]+"_NoAutoCorr_update_Jun30/v2_vs_pT/systematic_uncertainty/Centable/95percent_data_default_8pTbins_centralitytable_up/v24_"+label_centbin[icent]+"_abseta"+labelN_ybin[number_y]+"_sumw2/v24vspt_abseta"+labelN_ybin[number_y]+"_sumw2.root", "READ"); 
